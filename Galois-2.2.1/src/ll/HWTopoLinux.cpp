...
#ifdef GEM5
  // Do no open /proc/cpuinfo file in Gem5. Instead, do the following to
  // configure the cores
  // hardcoded to accept argument list starting like ./bin -t <threads/cores> ... 
  unsigned int maxGem5Cores = atoi(llvm::cl::arguments[2]); 
  
  for (unsigned id = 0; id < maxGem5Cores; id++)
  {
    cpuinfo info;
    info.proc = id;
    info.physid = 0;  // all cores on single package
    info.sib = maxGem5Cores;
    info.coreid = id;
    info.cpucores = maxGem5Cores; 
    vals.push_back(info);
  }
#else
